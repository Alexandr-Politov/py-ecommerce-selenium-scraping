...
def parse_single_product(element: WebElement) -> Product:
    return Product(
        ...
        .get_attribute("title"),
        description=(element.find_element(By.CSS_SELECTOR, ".description")
                     .text),
        price=float(element.find_element(By.CSS_SELECTOR, ".price")
                    .text.replace("$", "")),
...
def get_all_products() -> None:
    with webdriver.Chrome() as new_driver:
        ...
        for category, url in PAGES.items():
            page = load_page(url)
            elements = page.find_elements(By.CLASS_NAME, "thumbnail")
            products = [parse_single_product(element) for element in elements]
